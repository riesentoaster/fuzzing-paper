...
\begin{multicols}{2}
    ...
    \paragraph{Fuzzing: A Survey for Roadmap (\citeyear{FuzzingASurveyforRoadmap})}
    Similar to what is attempted in this paper, \textit{Fuzzing: A Survey for Roadmap} lists issues along common steps in fuzzing along with attempted solutions. It contains a short section about symbolic execution in the context input search space handling, but only discusses very few papers directly while often mentioning entire families of papers, including some that rely on symbolic execution, throughout the whole paper.\cite{FuzzingASurveyforRoadmap}
