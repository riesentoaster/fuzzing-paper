...
\begin{multicols}{2}
    ...
    In 2022, the cost of poor software quality was estimated to be more than \$2.4 trillion in the US alone.\cite{CostPoorSoftware} Individual cyber attacks have had estimated financial impacts of up to \$8 billion worldwide.\cite{Demystifying} Testing software amounts to approximately 50\% of the development cost\cite{DART}, but it is typically a mostly manual process\cite{PreliminaryAssessment}. Since manual testing requires a lot of developer hours with in-depth knowledge of the system to be tested, it does not scale well. Automated testing promises to be more cost-effective at finding software defects, and thus has become the most popular vulnerability discovery solution, especially in the industry.\cite{FuzzingASurvey}

    One such automated vulnerability and bug testing technique is fuzzing.\cite{VulnerabilityDiscoveryTechniques} In the seminal work by \citeauthor{UNIX} in \citeyear{UNIX}, the term \textquote{fuzz} is defined as a program that \textquote{generates a stream of random characters to be consumed by a target program}\cite{UNIX}. Since then, a rich ecosystem of fuzzing systems has developed in both industry and academia, drawing design inspiration from widely different software engineering concepts that combine into programs which produce different concrete inputs which are then repeatedly fed into a certain program under test (PUT) and then check the program for illegal states or crashes.\cite{EvaluatingFuzzTesting}

    Fuzzing is widely used in industry, which major technology companies and government agencies such as Google, Microsoft, the US Department of Defense, Cisco, and Adobe developing proprietary and contributing to open source fuzzers. These are then used to great effect, Google alone found 20K vulnerabilities in Chrome alone using fuzz testing.\cite{Demystifying}

    Symbolic execution is another approach to automated software testing. By not running programs with concrete inputs, but instead with variables representing all possible values. By tracking how these values are used, pure symbolic execution based systems can then reason about, and even proof certain hypotheses in, a PUT. However, because essentially needs to emulate the whole program including all possible program states, pure symbolic execution only works on trivial programs and breaks down with real-world programs because of their size. Naïve implementations further cannot handle non-trivial software that may be multi-threaded or interact with its environment.

    In the past decades, many fuzzers\cite{BitBlaze,CORAL,CREST,CUTE,CVC5,CVCLite,Chopped,Cyberdyne,DART,DTSA,DiSE,DigFuzz,Dowser,Driller,EGT,EXE,Fitnex,FloPSy,GRT,GSE,HCT,HFL,IFL,Intriguer,JFI,KATCH,KLEE,KLEEFP,LATEST,MoWF,Moles,PYGMALION,Pangolin,Pex,QSYM,QuickFuzz,RWset,SAGE,SAVIOR,SMART,SPIN,STP,ScalableAutomatedMethods,TFuzz,TaintScope,VUzzer} have employed symbolic execution based techniques.

    Since the academic research in this field is vast, existing survey papers are used to filter works on the topic at hand to the most important ones.

    In short, this paper makes the following contributions:
    \begin{itemize}
        \item First, it explains theoretical principles behind fuzzing and symbolic execution in section\ref{Theory}.
        \item Second, it gives an overview of existing survey papers investigating the state of fuzzing and provides a short summary of each in section\ref{Methods}.
        \item It then lists challenges fuzzing tools face when implementing symbolic execution techniques, and mitigation attempts at each, along examples of works that implement them in section\ref{Results}.
        \item Finally, it discusses limitations and possible additions to this work in section\ref{Discussion}.
    \end{itemize}
